...
def stream_joke(request):
    def event_stream():
        last_updated = None
        while True:
            try:
                joke_obj = CurrentJoke.objects.get(id=1)
                if last_updated != joke_obj.updated_at:
                    last_updated = joke_obj.updated_at
                    # Send Server-Sent Event formatted string
                    yield f"data: {joke_obj.text}\n\n"
            except CurrentJoke.DoesNotExist:
                # initial if no joke is generated by celery yet
                yield f"data: Waiting for Celery to generate first joke...\n\n"
                
            # Efficiently sleep waiting for Celery to update the joke
            time.sleep(1)
